...
func _handle_cleanup(aseprite_content, should_remove_spritesheet = false):
	if typeof(aseprite_content) != TYPE_ARRAY:
		aseprite_content = [aseprite_content]
	
	for content in aseprite_content:
		if config.should_remove_source_files():
			DirAccess.remove_absolute(content.data_file)
			if should_remove_spritesheet:
				DirAccess.remove_absolute(content.sprite_sheet)
				var import_file = "%s.import" % content.sprite_sheet
